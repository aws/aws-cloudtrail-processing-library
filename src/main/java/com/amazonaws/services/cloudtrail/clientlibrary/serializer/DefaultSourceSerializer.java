...
public class DefaultSourceSerializer implements AWSCloudTrailSourceSerializer {
    ...
    public CloudTrailSource getSource(Message sqsMessage) throws IOException {
        ...
        for (String objectKey : objectKeys) {
            accountId = accountId == null ? ClientLibraryUtils.extractAccountIdFromObjectKey(objectKey.toString()) : accountId;
            cloudTrailLogs.add(new CloudTrailLog(bucketName, objectKey));
        }

        //set accountId as message attribute
        sqsMessage.addAttributesEntry("accountId", accountId);
